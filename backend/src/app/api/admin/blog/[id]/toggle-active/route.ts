...
/**
 * Toggles the active state of a blog post atomically.
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await atomicToggleActive('blogPost', params.id, isActive);

    return NextResponse.json({ message: 'Post active state updated' });
  } catch (error) {
